...
class AdminControlActivity : AppCompatActivity() {
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (child in snapshot.children) {
                        ...
                        val accessibility = child.child("accessibility_granted").getValue(Boolean::class.java) ?: false
                        if (online) newList.add(DeviceInfo(id, name, model, online, lastSeen, lastStatus, lastStatusTime, accessibility))
                    }
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    for (child in snapshot.children) {
                        ...
                        newList.add(MediaItem(id, name, url))
                    }
                    songAdapter.updateItems(newList)
                }
                override fun onCancelled(error: DatabaseError) {}
            })
    }
        ...
        FirebaseDatabase.getInstance(DB_URL)
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    ...
                    wallpaperAdapter.updateItems(newList)
                }
                override fun onCancelled(error: DatabaseError) {}
            })
    }
    ...
    private fun listenForScreenshots() {
        FirebaseDatabase.getInstance(DB_URL)
            ...
            .addValueEventListener(object : ValueEventListener {
                override fun onDataChange(snapshot: DataSnapshot) {
                    val newList = mutableListOf<ScreenshotItem>()
                    for (child in snapshot.children) {
                        val id        = child.key ?: continue
                        val deviceId  = child.child("deviceId").getValue(String::class.java) ?: ""
                        val url       = child.child("url").getValue(String::class.java) ?: continue
                        val timestamp = child.child("timestamp").getValue(Long::class.java) ?: 0L
                        newList.add(ScreenshotItem(id, deviceId, url, timestamp))
                    }
                    screenshotAdapter.updateItems(newList)
                }
                override fun onCancelled(error: DatabaseError) {}
            })
    }
    ...
    private fun sendToSelected(action: (DatabaseReference) -> Unit) {
        ...
        selectedIds.forEach { id ->
            val db = FirebaseDatabase.getInstance(DB_URL).getReference("devices/$id")
            action(db)
        }
    ...
    private fun addMediaByUrl(type: String, name: String, url: String) {
        val ref = FirebaseDatabase.getInstance(DB_URL).getReference(type).push()
        ref.setValue(mapOf("name" to name, "url" to url))
    }
    ...
    private fun setupClickListeners() {
        ...
        btnTakeScreenshot.setOnClickListener {
            ...
            // Check karo accessibility enable hai ya nahi selected devices pe
            val noAccessibility = deviceList.filter {
                selectedIds.contains(it.id) && !it.accessibilityGranted
            }

            if (noAccessibility.isNotEmpty()) {
                val names = noAccessibility.joinToString(", ") { it.name.ifEmpty { it.model } }
                Toast.makeText(
                    this,
                    "⚠️ $names pe Accessibility Service enable nahi hai!\nSettings → Accessibility → DevSync Screenshot → ON karo",
                    Toast.LENGTH_LONG
                ).show()
                return@setOnClickListener
            }
